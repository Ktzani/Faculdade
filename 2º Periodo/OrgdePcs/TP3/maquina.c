...
void montarInstrucoes_Interrup(unidade_Instrucao *Instrucoes)
{
    srand(time(NULL));
    ...
    for (int i = 20; i < NRO_InstruInterru; i++)
    {
        if (rand() % 100 < REPETIR_INSTRUCAO)
        {
            Instrucoes[i] = Instrucoes[i - (1 + rand() % 15)];
    ...
    Instrucoes[NRO_InstruInterru].opCode = -1;
}
...
void maquinaInterpretada(blocoMemoria *cacheL1, blocoMemoria *cacheL2, blocoMemoria *cacheL3, blocoMemoria *RAM, unidade_Instrucao *vetorInstrucao, int *cache1_Hit, int *cache2_Hit, int *cache3_Hit, int *cache1_Miss, int *cache2_Miss, int *cache3_Miss, int *custoTotal, unidade_Instrucao *instruInterru, int *contInterru)
{
    ...
    for (int i = 0; i < NRO_INSTRUCOES; i++)
    {
        ...
        case 0:
            ...
            if(rand() % 100 < CHANCE_INTERRU && *contInterru < 3){

                printf (ANSI_BOLD ANSI_COLOR_BLUE "INICIO - TRADADOR DE INTERRUPÇÃO %d:\n" ANSI_RESET, contgeral);

                contInterru++;

                contgeral++;

                maquinaInterpretada(cacheL1, cacheL2, cacheL3, RAM, vetorInstrucao, cache1_Hit, cache2_Hit, cache3_Hit, cache1_Miss, cache2_Miss, cache3_Miss, custoTotal, instruInterru, contInterru);

                contInterru = 0;

                printf (ANSI_BOLD ANSI_COLOR_RED "FIM - TRADADOR DE INTERRUPÇÃO\n" ANSI_RESET);
            }
