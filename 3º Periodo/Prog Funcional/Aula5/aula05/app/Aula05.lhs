...
area (Triangle a b c) = sqrt (peri * (peri - a) * (peri - b) * (peri - c))
      where 
        peri = (dist (a,b) + dist (b,c) + dist (c,a)) / 2             
...
nElementTree :: IntTree -> Int
nElementTree ILeaf = error "Folha"
nElementTree (INode _ r l) = somEle
  where 
    somEle = 1 + nElementTree r
    somEle = 1 + nElementTree l

nLeafTree :: IntTree -> Int -> Int
nLeafTree n ILeaf = n + 1
nLeafTree (INode _ r l) = nLeafTree r n 
nLeafTree (INode _ r l) = nLeafTree l n 
...
convert :: IntList -> IntTree
convert INil = ILeaf
convert (ICons x xs) = criar_arvore (INode x ILeaf ILeaf) xs 
      where 
        criar_arvore tree [] = tree
        criar_arvore tree (ICons x xs) = criar_arvore (inserir tree x) xs

inserir ::  IntList -> Int -> IntList
inserir ILeaf x = (INode x ILeaf ILeaf)
inserir (INode arv1 v arv2) x 
  | (v == x) = INode arv1 v arv2
  | (v < x) = INode arv1 v (inserir arv2 x)
  | otherwise = INode (inserir arv1 x) v arv2
...
data Client = Customer Name Surname SendOffer

mensagem :: [Client] -> [String]
mensagem [] = []
mensagem (x:xs) 
  | x == Customer _ _ True = x : mensagem xs
  | otherwise = mensagem xs
